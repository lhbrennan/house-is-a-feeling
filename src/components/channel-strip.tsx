...
function PanSlider({ value, onChange }: { value: number; onChange: (value: number) => void }) {
    // Map value from [-1, 1] to a percentage (0% to 100%).
    // When value is -1 -> 0%, 0 -> 50%, 1 -> 100%
      ...
      <div className="w-32 relative py-4">
        ...
        >
          <RadixSlider.Track className="relative w-full h-2 rounded-full bg-gray-300">
            ...
            <div className="absolute h-full bg-blue-500 rounded-full" style={fillStyle} />
          </RadixSlider.Track>
